                        ...
                        for run in para.runs:
                            for i in range(0, len(old_texts)):
                                if run.text:  # 避免图片丢失
                                    run.text = run.text.replace(
                                        old_texts[i], new_texts[i])  # 替换信息
    fdocx.save(inPathName.split(".docx")[
               0]+"_"+str(time.time())+"_xiugai.docx")
...
pathtemp = "/Users/liudyixia/OneDrive/瑞数信息/20200420重庆移动/重庆移动信安部/重庆移动2021年服务项目投标/"
A = pathtemp + '瑞数-重庆移动2021年服务投标技术文件.docx'
B = pathtemp + '技术文件_BWD_V2.docx'
find_same_twof([A, B])
'''
fpath = "test.docx"
